...
void ModeAuto::wp_run()
{
    ...
    // --- START OF RC "NUDGE" LOGIC (Z-Axis "Přelet" Only) ---
    // This block is inserted *after* wp_nav has run.

    // 1. Get the original desired velocity vector from the AUTO navigation (cm/s, in NED frame)
    //    (Using the base alias from AC_PosControl.h)
    Vector3f vel_mission = copter.pos_control->get_vel_target();

    // 2. Read and normalize Throttle input
    const float throttle_input = copter.channel_throttle->get_control_in();
    const float throttle_mid = copter.channel_throttle->get_control_mid();

    // Normalize Throttle (-1.0 to 1.0, where -1.0 is full down)
    float throttle_nudge = constrain_float((throttle_input - throttle_mid) / throttle_mid, -1.0f, 1.0f);

    // 3. Define the maximum deviation speed (in cm/s)
    const float max_nudge_speed_z_cms = 100.0f; // 1 m/s for vertical movement

    // 4. Calculate the vertical (Down) component
    // Note: In NED, positive Z is DOWN. Throttle nudge is positive for UP (climb), so we negate it.
    float z_nudge_cms = -(throttle_nudge * max_nudge_speed_z_cms);

    // 5. Create the final velocity target.
    // Start with the original mission velocity (keeping X and Y intact).
    Vector3f final_vel_target = vel_mission;

    // 6. Apply *only* the Z-axis nudge.
    // We mix (add) the nudge to the mission's vertical speed.
    final_vel_target.z += z_nudge_cms;

    // 7. Send the final command to the position controller.
    // The X and Y (North/East) components remain untouched from the original mission.
    copter.pos_control->set_target_velocity_NED(final_vel_target);

    // IMPORTANT: Ensure that the original control flow does not call position control
    // setters again *after* this block with the un-nudged values.

    // --- END OF RC "NUDGE" LOGIC ---

    // // === Thrust‑nudge (Z offset) in cm/s ===
    // // Thrust‑nudge (Z offset) in cm/s
    // copter.auto_thr_nudge_update(G_Dt);
    // const float vz_cms = copter._thr.vz_cms;  // už saturováno v update

    // // Apply as climb-rate command into vertical position controller
    // // Signature (typicky): set_pos_target_z_from_climb_rate_cm(float climb_rate_cms, float dt, bool force_descend)
    // pos_control->set_pos_target_z_from_climb_rate_cm(vz_cms, G_Dt, false);
